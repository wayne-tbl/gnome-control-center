...
struct _CcWwanModeRow
{
  ...
  MMModem *modem;
  guint band_val;
  CcWwanModeDialog *parent_dialog;
};
...
static void
cc_wwan_mode_dialog_row_toggle_cb (GtkCheckButton *button,
                                   CcWwanModeRow *row)
{
  guint n_bands;
  GList *l;
...
static void
cc_wwan_mode_dialog_row_activated_cb (CcWwanModeRow *row,
                                      gint           n_press,
                                      gdouble        x,
                                      gdouble        y)
{
                             ...
                             MMModem                  *modem)
{
  ...
  g_signal_connect (checkbox, "toggled",
                    G_CALLBACK (cc_wwan_mode_dialog_row_toggle_cb), row);
  ...
  g_signal_connect_swapped (listener, "released",
                    G_CALLBACK (cc_wwan_mode_dialog_row_activated_cb), row);
...
cc_wwan_mode_dialog_update (CcWwanModeDialog *self)
{
  ...
  for (i = 0; i < G_N_ELEMENTS (bands[0]); i++)
    {
      GtkWidget *row;
      gboolean is_supported = false;
      gboolean is_enabled = false;

      if (bands[0][i].name == NULL)
        break;

      // If we don't support this band, skip it
      for (size_t j = 0; j < n_supported_bands; j++)
        {
          if (supported_bands[j] == bands[0][i].struct_val)
            {
              is_supported = true;
              break;
            }
      ...
      for (size_t j = 0; j < n_enabled_bands; j++)
        {
          if (enabled_bands[j] == bands[0][i].struct_val)
            {
              is_enabled = true;
              break;
            }
      ...
      row = cc_wwan_mode_dialog_row_new (self, bands[0][i], is_enabled, modem);
      gtk_list_box_append (GTK_LIST_BOX (self->twog_band_list), row);
    }

  for (i = 0; i < G_N_ELEMENTS (bands[1]); i++)
    {
      GtkWidget *row;
      gboolean is_supported = false;
      gboolean is_enabled = false;

      if (bands[1][i].name == NULL)
        break;
      ...
      for (size_t j = 0; j < n_supported_bands; j++)
        {
          if (supported_bands[j] == bands[1][i].struct_val)
            {
              is_supported = true;
              break;
            }
      ...
      for (size_t j = 0; j < n_enabled_bands; j++)
        {
          if (enabled_bands[j] == bands[1][i].struct_val)
            {
              is_enabled = true;
              break;
            }
      ...
      row = cc_wwan_mode_dialog_row_new (self, bands[1][i], is_enabled, modem);
      gtk_list_box_append (GTK_LIST_BOX (self->threeg_band_list), row);
    }

  for (i = 0; i < G_N_ELEMENTS (bands[2]); i++)
    {
      GtkWidget *row;
      gboolean is_supported = false;
      gboolean is_enabled = false;

      if (bands[2][i].name == NULL)
        break;

      // If we don't support this band, skip it
      for (size_t j = 0; j < n_supported_bands; j++)
        {
          if (supported_bands[j] == bands[2][i].struct_val)
            {
              is_supported = true;
              break;
            }
      ...
      for (size_t j = 0; j < n_enabled_bands; j++)
        {
          if (enabled_bands[j] == bands[2][i].struct_val)
            {
              is_enabled = true;
              break;
            }
      ...
      row = cc_wwan_mode_dialog_row_new (self, bands[2][i], is_enabled, modem);
      gtk_list_box_append (GTK_LIST_BOX (self->fourg_band_list), row);
    }

  for (i = 0; i < G_N_ELEMENTS (bands[3]); i++)
    {
      GtkWidget *row;
      gboolean is_supported = false;
      gboolean is_enabled = false;

      if (bands[3][i].name == NULL)
        break;

      // If we don't support this band, skip it
      for (size_t j = 0; j < n_supported_bands; j++)
        {
          if (supported_bands[j] == bands[3][i].struct_val)
            {
              is_supported = true;
              break;
            }
      ...
      for (size_t j = 0; j < n_enabled_bands; j++)
        {
          if (enabled_bands[j] == bands[3][i].struct_val)
            {
              is_enabled = true;
              break;
            }
      ...
      row = cc_wwan_mode_dialog_row_new (self, bands[3][i], is_enabled, modem);
      gtk_list_box_append (GTK_LIST_BOX (self->fiveg_band_list), row);
    }
}
...
cc_wwan_mode_dialog_class_init (CcWwanModeDialogClass *klass)
{
  ...
  gtk_widget_class_bind_template_child (widget_class, CcWwanModeDialog, twog_band_list);
  gtk_widget_class_bind_template_child (widget_class, CcWwanModeDialog, threeg_band_list);
  gtk_widget_class_bind_template_child (widget_class, CcWwanModeDialog, fourg_band_list);
  gtk_widget_class_bind_template_child (widget_class, CcWwanModeDialog, fiveg_band_list);
